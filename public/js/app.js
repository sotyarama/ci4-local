(function () {
    ...
    function setupFilter(opts) {
        ...
        const nores = opts.noResult
            ? typeof opts.noResult === 'string'
                ? document.querySelector(opts.noResult)
                : opts.noResult
            : null;
    ...
    (function initSidebarCollapse() {
        ...
        if (saved) {
            ...
            // default: keep only 'main' and 'master' expanded; collapse other groups
            collapseState = {
                transactions: true,
                inventory: true,
                reports: true,
        ...
        function setCollapsedState(title, collapsed) {
            const section = title?.getAttribute('data-target');
            if (!section) return;
            const group = document.getElementById('nav-' + section);
            if (collapsed) {
                title.classList.add('collapsed');
                title.setAttribute('aria-expanded', 'false');
                if (group) group.setAttribute('aria-hidden', 'true');
                collapseState[section] = true;
            } else {
                title.classList.remove('collapsed');
                title.setAttribute('aria-expanded', 'true');
                if (group) group.setAttribute('aria-hidden', 'false');
        ...
        function toggleSection(section, force) {
            ...
            const wantCollapse = force !== undefined ? !!force : !isCollapsed;
            setCollapsedState(title, wantCollapse);
        }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('.nav-section-title.collapsible').forEach(function (title) {
                ...
                if (collapseState[target] === true) {
                    setCollapsedState(title, true);
                } else {
                    setCollapsedState(title, false);
                }
